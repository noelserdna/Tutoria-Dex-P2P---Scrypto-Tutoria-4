...
resim transfer 100000,$usd $acct2
------------------------------------------------------------------------------------------------
...
blueprint! {
    ...
    impl DexP2p {
        pub fn new(fee: Decimal) -> (Component, Bucket) {
            // Credencial para administrador del componente
            ...
            // Credencial mantenida por el componente para acuñar NFT de identificación
            ...
            // NFT utilizado para identificar ordenes
        ...
        pub fn ejecutar_orden(&mut self, identificador: NonFungibleKey, mut pago: Bucket, mut fee_xrd: Bucket) -> (Bucket, Bucket, Bucket) {
            ...
            let id: NonFungibleKey = identificador.clone(); // pierdo el dato identificado y por eso lo clono
            //vendedor
            self.registro_ventas.insert(identificador, Vault::with_bucket(pago.take(cantidad)));  
            //comprador
            let badge = self.identification_minter.authorize(|auth| {
            ...
            self.identification_minter
                .authorize(|auth| self.identification_nft_def.update_non_fungible_data(&id, OrdenData{retirado: false, vendido: true} , auth));
